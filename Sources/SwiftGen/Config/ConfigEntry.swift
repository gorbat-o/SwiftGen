...
extension Config.Entry {
  init(yaml: [String: Any], cmd: String, logger: (LogLevel, String) -> Void) throws {
    ...
    if let outputs = yaml[Keys.outputs] {
      do {
        self.outputs = try Config.Entry.Output.parseCommandOutput(yaml: outputs, logger: logger)
      ...
      self.outputs = try Config.Entry.Output.parseCommandOutput(yaml: yaml, logger: logger)
    } else {
  ...
  static func parseCommandEntry(yaml: Any, cmd: String, logger: (LogLevel, String) -> Void) throws -> [Config.Entry] {
    return try Config.Entry.parseValueOrArray(yaml: yaml) {
      return try Config.Entry(yaml: $0, cmd: cmd, logger: logger)
    }
  }
